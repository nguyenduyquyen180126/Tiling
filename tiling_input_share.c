...
int main(){
    ...
    // ===== Loop dể tính ofm =====
    /*  Vẫn load weight trước như phần tiling weight stationary*/
    int num_passes = ceil((float)INPUT_C / PARALLEL_CHANNELS);
    for(int p = 0; p < num_passes; p++){
        load_w_share(&w, w_buffer, p);                       // Dùng hàm load kiểu input share
        for(int ho = 0; ho < OUTPUT_H; ho++){
            for(int wo = 0; wo < OUTPUT_W; wo++){
                load_ifm_share(&ifm, ifm_buffer, ho, wo, p); // Dùng hàm load kiểu input share
                int32_t p_sum = runPE_Array(&pe_array);
    ...
    // Tính số chu kỳ cho việc load buffer
